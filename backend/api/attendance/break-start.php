<?php
// ==========================================================
// MGI ERP / HRIS - Attendance Break Start (Awal Istirahat) API
// ==========================================================
...
try {
    ...
    if ($attendance['break_start']) {
        sendError('Anda sudah melakukan presensi awal istirahat hari ini.', 400);
    }
    ...
    // Update attendance record with break_start
    $stmt = $pdo->prepare("UPDATE attendances SET break_start = ?, break_start_latitude = ?, break_start_longitude = ?, break_start_accuracy = ?, break_start_distance = ? WHERE id = ?");
    $stmt->execute([$currentTime, $lat, $lng, $accuracy, $distance, $attendance['id']]);

    echo json_encode([
        'success' => true,
        'message' => 'Presensi awal istirahat berhasil',
        'data' => [
            'type' => 'break_start',
            'time' => $currentTime,
            'distance' => round($distance),
    ...
    sendError('Presensi awal istirahat gagal: ' . $e->getMessage(), 500);
}
